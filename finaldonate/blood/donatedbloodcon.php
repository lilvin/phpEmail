...
//add record
 ...
 $serial = $_POST['serial'];
   ...
   $hospital = $_POST['hospital'];
    $ID = $_POST['ID'];
	$gender = $_POST['gender'];
	  if(!empty($ID)&& !empty($serial)  && !empty($ddate) && !empty($hospital)){
	   
	   $sql = "INSERT INTO donatedblood(ID,gender,serial,hospital,ddate) VALUES ('$ID','$gender','$serial','$hospital','$ddate')";
    ...
    <table width="332" border="1" align="center">
      ...
      <tr>
	   ...
	   <tr>
        ...
        <td><input name="ID" maxlength="8" type="text" id="ID" value="<?php echo $ID; ?>">
                        </input></td>
      </tr>
	   <tr>
...
<?php 
 ...
 $serial =$_POST['serial'];
  ...
  $ddate =$_POST['ddate'];
    ...
    if(!empty($ID) && !empty($ddate) && !empty($serial) && !empty($gender) && !empty($hospital)){
 $query= "UPDATE donatedblood SET ID='$ID', serial='$serial',hospital='$hospital', ddate='$ddate', gender='$gender' WHERE serial='$serial' ";
if ($query_run= mysqli_query($con,$query)){
